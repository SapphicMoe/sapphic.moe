...
You'll need to install Docker first. The official Docker Docs page provides [a guide][docker-install] on how to install
Docker on just about every system.

Then, head over to Owncloud's [documentation page][owncloud-install] and install Owncloud on a Docker container.

## Creating an R2 bucket

Head over to your [Cloudflare dashboard][cloudflare-dash] and navigate to the R2 page.  
...
![R2 API tokens link](https://i.elixi.re/i/4281o.png 'R2 API tokens link')
...
- **Token name**: This is not that important, but if you want to be able to easily identify the R2 token on this page in
  ...
  ![Token name option](https://elixi.re/i/lqn7h.png 'Token name option')
...
![S3 extension](https://elixi.re/i/8xj9r.png 'S3 extension')
...
```bash
root@owncloud:/var/www/owncloud/config# ls
...
- **hostname**: You can find the bucket hostname on the bucket page itself.
  ![R2 bucket hostname](https://elixi.re/i/y8e7l.png 'R2 bucket hostname')
